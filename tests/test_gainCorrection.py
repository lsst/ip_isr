...
class GainCorrectionTest(lsst.utils.tests.TestCase):
    ...
    def testCorrectGains(self):
        gains = np.array([1.5, 1.5])
        correctedGains = self.gainCorrection.correctGains(gains)

        correctedGains2 = gains * self.gainCorrection.gainAdjustments

        np.testing.assert_array_almost_equal(correctedGains, correctedGains2)
