...
class ElectrostaticBrighterFatterCalibration(IsrCalib):
    ...
    **kwargs :
    ...
    def fromDict(cls, dictionary):
        ...
        for ampName in dictionary['ampNames']:
            ...
            calib.aMatrix[ampName] = np.array(
                ...
                dtype=np.float64).reshape((inputRange, inputRange),
            )
            ...
            calib.aMatrixModel[ampName] = np.array(
                dictionary['aMatrixSigma'][ampName],
                dtype=np.float64).reshape((fitRange, fitRange),
            )
    ...
    def toDict(self):
        ...
        outDict['fitMask'] = dictOfArraysToDictOfLists(self.fitMask)
    ...
    def fromTable(cls, tableList):
        ...
        # Initialize inDict with all expected keys and empty values of the corresponding type
        inDict = dict()
    ...
    def toTable(self):
        ...
        for ampName in self.ampNames:
            ampDict = {
                ...
                'A_MATRIX': self.aMatrix.get(ampName).ravel() if ampName in self.aMatrix else None,
                'A_MATRIX_SIGMA': self.aMatrixSigma.get(ampName).ravel() if ampName in self.aMatrixSigma else None,
                'A_MATRIX_MODEL': self.aMatrixModel.get(ampName).ravel() if ampName in self.aMatrixModel else None,
                'A_MATRIX_SUM': self.aMatrixSum.get(ampName),
                'A_MATRIX_MODEL_SUM': self.aMatrixModelSum.get(ampName),
                'MODEL_NORMALIZATION': self.modelNormalization.get(ampName).ravel() if ampName in self.modelNormalization else None,
                'FIT_MASK': self.fitMask.get(ampName).ravel() if ampName in self.fitMask else None,
                'FIT_PARAM_NAMES': self.fitParamNames.ravel() if ampName in self.fitParamNames else None,
                'FREE_FIT_PARAM_NAMES': self.freeFitParamNames.ravel() if ampName in self.freeFitParamNames else None,
                'FIT_PARAMS': self.fitParams.get(ampName).ravel() if ampName in self.fitParams else None,
                'FIT_PARAM_ERRORS': self.fitParamErrors.get(ampName).ravel() if ampName in self.fitParamErrors else None,
                'FIT_CHI2': self.fitChi2.get(ampName),
                'FIT_REDUCED_CHI2': self.fitReducedChi2.get(ampName),
                'FIT_PARAM_COV_MATRIX': self.fitParamCovMatrix.get(ampName).ravel() if ampName in self.fitParamCovMatrix else None,
                'ATH': self.ath.get(ampName).ravel() if ampName in self.ath else None,
                'ATH_MINUS_BETA': self.athMinusBeta.get(ampName).ravel() if ampName in self.athMinusBeta else None,
                'A_N': self.aN.get(ampName).ravel() if ampName in self.aN else None,
                'A_S': self.aS.get(ampName).ravel() if ampName in self.aS else None,
                'A_E': self.aE.get(ampName).ravel() if ampName in self.aE else None,
                'A_W': self.aW.get(ampName).ravel() if ampName in self.aW else None,
            }
