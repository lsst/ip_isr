"""
@brief A simple test for the ISR stage, 'Ilumination Correction'.
...
inMasterSFFilePath = os.path.join(dataDir, "CFHT", "D4", "05Am05.NSflat.0.36.00_1")
...
class isrTestCases(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.masterSFChunkExposure = afwImage.ExposureF()
        self.masterSFChunkExposure.readFits(inMasterSFFilePath)
        self.isrPolicy = pexPolicy.Policy.createPolicy(isrPolicyPath)

    def tearDown(self):
        del self.masterChunkExposure
        del self.masterSFChunkExposure
        del self.isrPolicy

    def testBiasCorrection(self):
        
        ipIsrFlat.flatFieldCorrection(self.masterChunkExposure, self.masterSFChunkExposure, self.isrPolicy)
        self.masterChunkExposure.writeFits(outputPath)    
