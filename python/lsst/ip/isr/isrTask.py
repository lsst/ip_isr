...
class IsrTask(pipeBase.PipelineTask):
    ...
    def run(self, ccdExposure, *, camera=None, bias=None, linearizer=None,
        ...
        * doFlat
        ...
        exposureMetadata["LSST ISR BFK APPLIED"] = False
        ...
        if self.config.doBrighterFatter:
            ...
            ccdExposure.metadata["LSST ISR BFK APPLIED"] = True
    ...
    def defineEffectivePtc(self, ptcDataset, detector, bfGains, overScans, metadata):
        ...
        for amp, overscanResults in zip(amps, overScans):
            ...
            if self.config.doEmpiricalReadNoise and overscanResults is not None:
                ...
                # the read noise attributes of all
                ...
                # We already have a detector at this point. If there was no
                # detector to begin with, one would have been created with
                # self.config.gain and self.config.noise.
                # Amplifier object read noise is always in electron
            ...
            # Make sure read noise, turnoff, and gain make sense
