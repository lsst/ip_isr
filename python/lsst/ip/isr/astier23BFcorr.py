...
class CustomFFTConvolution(object):
    ...
    def __init__(self, A, B, threads=1):
        ...
        self.ifft_plan = pyfftw.builders.irfftn(
                    self.fft_plan_im.output_array, s=shape,
                    threads=threads)
...
class BFCorr:
    ...
    determination of pixel boundary shifts under the influcenc of a
    single electron)
    ...
    def DeltaImageFFT(self, im):
        ...
        delta[:,1:] -=  dq[:,:-1]
        # one might check that res.sum() ~ 0 (charge conservation)
        return delta
