...
class CustomFFTConvolution(object):
    ...
    def __init__(self, im, kernel, threads=1):
        ...
        # Find the next larger "fast size" for FFT computation. This can provide a significant speedup.
        shape = np.array([pyfftw.next_fast_len(s) for s in shape])
        # Create FFTW plans for the image and kernel.
    ...
    def __call__(self, im, kernels):
        """
        Perform the convolution and trim the result to the size of the input image.
        If kernels is a list, then the routine returns a list of corresponding convolutions.
        """
        # Handle both a list of kernels and a single kernel.
        ks = [kernels] if type(kernels) != list else kernels
        convolutions = []
        for k in ks:
        ...
        # Return a single convolution if kernels was not a list, otherwise return the list.
        return convolutions[0] if type(kernels) != list else convolutions
...
def electrostaticBrighterFatterCorrection(exposure, ebf, applyGain, gains=None):
    ...
    kN[:, 0] = -kN[:, -1]
    kE[0, :] = -kE[-1, :]
    # TODO: Add to log: print("INFO: BF kernel sum rules : kN %f, kE %f" % (kN.sum(), kE.sum()))
